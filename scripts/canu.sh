...
# Run Canu.
canu -p canu -d "$temp_dir" -fast genomeSize="$genome_size" useGrid=false maxThreads="$threads" -pacbio-raw "$reads"
...
# Trim the contigs.
/home/nik_arapitsas/Documents/Bacillus_project/scripts/canu_trim.py "$temp_dir"/canu.contigs.fasta > "$assembly".fasta
