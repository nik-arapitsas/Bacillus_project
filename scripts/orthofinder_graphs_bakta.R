...
data_gene_all_any$Isolates <- factor(data_gene_all_any$Isolates, levels = data$Isolates)
data_gene_all_any <- data_gene_all_any[order(data_gene_all_any$Isolates), ]
...
SharedOgs_plot <- ggplot(data_combined, aes(x = Isolates, y = Count, fill = Category)) +
  geom_bar(stat = "identity", position = "stack") +  # Maintain layering
  scale_fill_manual(values = c("Partially.Shared.Orthogroups" = "#7d92e7", 
                               "Core.Orthogroups" = "#f8d48c"),
                    labels = c("Partially.Shared.Orthogroups" = "Partially Shared Orthogroups",
      "Core.Orthogroups" = "Core Orthogroups")) +
  labs(title = "Number of Orthogroups per Isolate", y = "", x = "", fill = "Category") +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.margin = margin(-2, 18, 4, 1.2),  # Reduce legend margins
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),  # Center align title
...
collective_orthofinder_graph <- ggarrange(IsolateSpecificOgs_plot, PercOfOgs_plot, SharedOgs_plot,
          labels = c("A", "B", "C"),
          font.label = list(family = "Helvetica", size = 18, face = "plain"),                                
          ncol = 1, 
          nrow = 3, 
